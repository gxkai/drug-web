<template>
  <div>
    ...
    <div class="info-wrapper">
      <info-cell :desc="`${shopDrug.sfda || '暂无介绍'}`" title="国药准字" />
      <info-cell :desc="`${shopDrug.form || '暂无介绍'}`" title="剂型/型号" />
      <info-cell
        :desc="`${shopDrug.origin.name || '暂无介绍'}`"
        title="生产厂商"
      />
      <info-cell :desc="`${shopDrug.spec || '暂无介绍'}`" title="包装规格" />
    </div>
    <new-white-space size="0.1rem" />
    <div class="shop-wrapper hairline-bottom">
      ...
      <div class="right">
        <span @click="loadPageShopsView(shopDrug.shopId)">进入药店</span>
      </div>
    </div>
    ...
    <div class="drug-appraise-wrapper">
      ...
      <article>
        <appraise-cell
          v-for="(item, index) in shopDrug.drugAppraises.list"
          :key="index"
          :item="item"
        />
      </article>
