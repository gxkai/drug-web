    ...
    <template slot="center">
      ...
      <van-pull-refresh v-model="isLoading" @refresh="onRefresh">
        <van-list
          ...
          :finished="finished"
          @load="onLoad">
          <div v-for="item in list" @click="onRead(item)" class="hairline-bottom">
            <div class="wrapper">
                <div class="item1">
                ...
                <div class="item3">
                  <span>{{item.createdDate|dateFmt("YYYY-MM-DD hh:mm:ss")}}</span>
                </div>
            </div>
          </div>
  ...
  export default {
    ...
    methods: {
      async onLoad() {
        ...
        const data = await this.$http.get('/api/messages', this.getParams(params));
