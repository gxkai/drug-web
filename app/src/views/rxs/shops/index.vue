<template>
  <new-layout>
    ...
    <template slot="center">
      <new-white-space/>
      <new-hospital :item="hospital" v-if="hospital !== null"/>
      <div v-for="(item, index) in rxShops" :key="index">
        <new-white-space/>
        <new-shop :item="item" @click.native="clickItem(item)"/>
      </div>
      <new-end name="没有符合条件的药店" v-if="rxShops.length===0"/>
    </template>
  </new-layout>
</template>
...
<script>
  export default {
    ...
    methods: {
      ...
      async getRxShops() {
        this.rxShops_ = await this.$http.get(`/api/rxs/${this.rxId}/shops?lng=${this.currentAddress.lng}&lat=${this.currentAddress.lat}`);
        this.rxShops = this._.cloneDeep(this.rxShops_);
      },
      async getHospital() {
        this.hospital = await this.$http.get(`/api/rxs/${this.rxId}/hospital?lng=${this.currentAddress.lng}&lat=${this.currentAddress.lat}`);
      },
      clickItem(item) {
