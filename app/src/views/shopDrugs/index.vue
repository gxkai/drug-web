    ...
    <new-layout>
      <template slot="center">
        <van-tabs v-model="active">
          <van-tab title="商品">
            <component is="goods" :shopDrug="shopDrug"/>
          </van-tab>
          <van-tab title="详情">
            <component is="info" :shopDrug="shopDrug"/>
          </van-tab>
          <van-tab title="评价">
            <new-white-space/>
            <component is="appraise" :shopDrug="shopDrug"/>
          </van-tab>
        </van-tabs>
      </template>
      <template slot="bottom">
        <van-goods-action style="position: sticky">
          ...
          <van-goods-action-mini-btn icon="pingjia-1" text="收藏" :style="{color: collected === true ? 'red': ''}"
                                     @click="onCollect"/>
          <div class="submit-buttons">
            <div @click="onShow(0)" :style="{backgroundColor:shopDrug.otc===true && shopDrug.stock > 0?'#f02b2b':'gray'}">
              <span>
              加入购物车
            </span>
            </div>
            <div @click="onShow(1)" :style="{backgroundColor:shopDrug.otc===true && shopDrug.stock > 0?'#f0912b':'gray'}">
               <span>
              立即购买
            </span>
            </div>
  ...
  export default {
    ...
    methods: {
      ...
      async initData() {
        ...
        this.collected = await this.$http.get(`/api/collects/drug/one?shopDrugId=${this.shopDrugId}`);
      },
      async onCollect() {
        let data = {
