              ...
              <div class="cart-list-drugs-item">
                ...
                <div class="cart-list-drugs-item-right">
                  ...
                  <div>
                    ...
                    <div>
                      <span class="cart-list-drugs-item-right_quantity">x{{cartDrug.quantity}}</span>
                    </div>
                  </div>
                </div>
              </div>
  ...
  export default {
    ...
    methods: {
      async getData() {
        ...
        console.log(this.cartShops.filter(e => e.radio).length);
      },
      /**
       * 单个删除
       */
      ...
      async onOrder() {
        ...
        if (this.cartShops.filter(e => e.radio).length > 1) {
          this.$toast('最多选择一家药店预结算');
          return;
        }
        try {
          this.loading = true;
          const data = await this.$http.get(`/orders/cart?cartIds=${cartIds}&isRx=true`);
          this.$router.push({ path: '/orders/create/fromCart', query: { cart: JSON.stringify(data) } });
        } catch (e) {
          this.loading = false;
          this.$toast.fail('预结失败');
        }
      },
      /**
       * 选中raido
